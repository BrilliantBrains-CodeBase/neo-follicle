 ...
 *     made.
 *   - ALL NINE CARDS NOW CARRY AN IMAGE. GFC, QR678 and Exosome had none when
 *     this section was written and ran on a flat `bg-secondary` ground. Their
 *     cards now use the same hero each treatment page shows, so a card and the
 *     page it leads to open on the same picture. Those heroes come from the
 *     client's illustration set (content/Zoho WorkDrive-3/, see
 *     scripts/treatment-assets.mjs); they replaced smiling-portrait stock from
 *     the old pages, which did not depict the treatment and were near-identical
 *     to one another.
 *
 *     THE GRID STILL MIXES TWO STYLES. FLAGGED FOR CLIENT REVIEW. PRP, GFC,
 *     QR678 and Exosome use the illustrations; the other five cards still use
 *     the home page's photographs (public/services/). The illustration set has a
 *     matching image for every one of them, so swapping them would make the grid
 *     uniform -- left alone here because it was not asked for and changes the
 *     home page's cards too. PRP was moved across separately: it was on the
 *     gallery's before/after photograph, whose baked-in "Before / After"
 *     labels ran into the card title.
 *
 *     The `img`-less branch below is deliberately KEPT. A tenth category, or a
 *     treatment whose capture has no usable photograph, would otherwise render
 *     a broken image; the flat ground is the fallback for that case.
 *
 ...
 *
 * IMAGE PROVENANCE. The five under public/services/ are the home page's own
 * service cards (four licensed stock carried over from the live site,
 * /services/female.webp client-supplied); the four under public/treatments/ are
 * the client's illustrations, generated by scripts/gen-treatment-images.mjs from
 * content/Zoho WorkDrive-3/. Alt text describes the image only and claims no
 * result.
 */
const CATEGORIES = [
  {
  ...
  },
  {
    ...
    to: '#regenerative',
    img: '/treatments/best-prp-hair-treatment-in-bangalore/hero.webp',
  ...
  {
    title: 'GFC Therapy',
    copy: 'Growth-factor treatment for thinning hair.',
    to: '#regenerative',
    img: '/treatments/gfc-hair-treatment-in-bangalore/hero.webp',
    alt: 'A clinician injecting a reclining man’s scalp, with insets of a blood sample tube and a hair follicle.',
  },
  {
    title: 'QR678 Therapy',
    copy: 'Advanced peptide-based treatment.',
    to: '#regenerative',
    img: '/treatments/qr678-hair-treatment-in-bangalore/hero.webp',
    alt: 'A clinician injecting a reclining man’s scalp, with an inset of a vial of QR678.',
  },
  {
    title: 'Exosome Therapy',
    copy: 'Regenerative hair restoration.',
    to: '#regenerative',
    img: '/treatments/exosome-hair-treatment-in-bangalore/hero.webp',
